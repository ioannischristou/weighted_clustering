...
public class SCPMaker2 implements SCPMakerIntf {
 ...
 public void addOverExistingColumns(Vector clusters,
   ...
   * @param clusters Vector<Vector<Integer docid> cluster>
   * @param scpsolution int[]
   * @param double[] weights maybe null unused
   * @throws PartitioningException
   */
  public void addOverExistingColumns(Vector clusters,
                                     int[] scpsolution,
    ...
    try {
      ...
      for (int i=0; i<clusters.size(); i++) {
        ...
        while (cur_col<scpsolution.length && scpsolution[cur_col]==0) cur_col++;
        ...
        Vector centerjcosted = Document.getCenterCosted(cidocs, _m, null);
        double cost = ((Double) centerjcosted.elementAt(1)).doubleValue();
        _c.setQuick(cur_col, cost);
        cur_col++;
      }
