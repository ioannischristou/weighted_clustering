...
public class SCPMakerPM implements SCPMakerIntf {
    ...
    try {
      ...
      for (int i = 0; use_clusters_only==false && i < docsize && column_counter < _A.columns(); i++) {
        ...
        for (j = 1; j <= clusters_per_doc && column_counter < _A.columns(); j++) {
          ...
          // compute cost
          Vector r = Document.getDocumentCenterCostedInds(nnborids, _docs, _m);
      ...
      if (clusters!=null) {
        ...
        for (int j=0; j<clusters.size(); j++) {
          ...
          for (int l=0; l<clusterj.size(); l++) {
            ...
            cjdocinds.addElement(did);
          }
          Vector centerjcosted = Document.getDocumentCenterCostedInds(cjdocinds, _docs, _m);
    ...
    try {
      ...
      for (int i=0; i<X.size(); i++) {
        if (X.getQuick(i)==1.0) {
          ...
          Vector Xdocinds = new Vector();
          for (int j=0; j<_A.rows(); j++) {
            if (_A.getQuick(j,i)>0) {
              Xdocs.addElement(_docs.elementAt(j));
              Xdocinds.addElement(new Integer(j));
            }
          }
          Document center = Document.getDocumentCenterInds(Xdocinds, _docs, _m);
  ...
  public void addOverExistingColumns(Vector clusters,
    ...
    try {
      ...
      for (int i=0; i<clusters.size(); i++) {
          ...
          if (i==0 &&
            ...
            if (docs2move.size() > 0) {
              ...
              if (cur_col2 < _A.columns() && cont == false) {
                ...
                // Vector centerjcosted = Document.getDocumentCenterCosted(cidocs, _docs, _m);
                Vector centerjcosted = Document.getDocumentCenterCostedInds(iidocs, _docs, _m);
                ...
                centerjcosted = Document.getDocumentCenterCosted(docs2move, _docs, _m);  // itc: HERE expensive...
        ...
        // Vector centerjcosted = Document.getDocumentCenterCosted(cidocs, _docs, _m);
        Vector centerjcosted = Document.getDocumentCenterCostedInds(clusteri, _docs, _m);
        double cost = ((Double) centerjcosted.elementAt(1)).doubleValue();
        _c.setQuick(cur_col, cost);
        cur_col++;
      }
