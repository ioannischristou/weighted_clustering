...
import clustering.KMeansSqrGuarantClusterer;
import clustering.DocumentDistL2Sqr;
import clustering.KMeansSqrEvaluator;
...
public class SCPMaker1 implements SCPMakerIntf {
  protected Vector _docs;
  protected int _k;
  ...
  protected DocumentDistIntf _m;
  protected int[] _prevSolutions;  // _prevSolutions[i] is the number of times
  ...
  public void createSCP(Vector docs, Vector clusters, int k,
    ...
    try {
      ...
      System.err.println("constructing Matrix _A["+rows+" x "+cols+"]");  // itc: HERE rm asap
      ...
      for (int i = 0; use_clusters_only==false && i < docsize && column_counter < _A.columns(); i++) {
        ...
        if (helper==null) helper = new SCPMakerHelper(_docs, _m);
        // create helper only when needed
        for (j = 1; j <= clusters_per_doc && column_counter < _A.columns(); j++) {
          ...
          for (int jj=0; jj<nnborids.size(); jj++) {
            ...
            if (weights!=null) {
                wgtsi.add(new Double(weights[ii2.intValue()]));
            }
            _A.setQuick(ii2.intValue(), column_counter, 1);
          }
          // compute cost
          // if the cost of each column evaluator has been passed in, use it
          // else use standard k-means cost
          Evaluator costseval = (Evaluator) getParam("evaluator");
          if (costseval!=null && costseval instanceof EnhancedEvaluator) {
            double costj = ((EnhancedEvaluator) costseval).evalCluster(nnborids);
            _c.setQuick(column_counter,costj);
          }
          else {
            double[] ws = wgtsi.size()>0 ? new double[wgtsi.size()] : null;
            if (ws!=null) {
                for (int p=0; p<ws.length; p++) ws[p] = ((Double)wgtsi.get(p)).doubleValue();
            }
            Vector r = Document.getCenterCosted(nnbors, _m, ws);
        ...
        for (int jj=0; jj<docsize; jj++) {
          ...
          if (_m.dist(djj, max_cluster_center) < 1.25*avgdist || chance<0.0001) {
            ...
            if (weights!=null) wgtsi.add(new Double(weights[jji.intValue()]));
            nnborids.add(jji);
          }
        }
        // set _A
        ...
        // set _c
        // if the cost evaluator has been passed in, use it
        // else use standard k-means cost
        Evaluator costseval = (Evaluator) getParam("evaluator");
        if (costseval!=null && costseval instanceof EnhancedEvaluator) {
          double costj = ((EnhancedEvaluator) costseval).evalCluster(nnborids);
          ...
          if (wsi!=null) {
            for (int p=0; p<wsi.length; p++) wsi[p] = ((Double)wgtsi.get(p)).doubleValue();
          }
          Vector r = Document.getCenterCosted(nnbors, _m, wsi);
          cost = ( (Double) r.elementAt(1)).doubleValue();
          max_cluster_center = (Document) r.elementAt(0);
          _c.setQuick(column_counter, cost);
        }
      ...
      if (clusters!=null) {
        ...
        ArrayList wgtsi = new ArrayList();
        // check if clusters that contain points for which their p-NN are
        // not also contained in the cluster should be heavily penalized
        Integer p = (Integer) _params.get("nn_in_cluster_threshhold");
        if (p!=null) {
          if (helper==null) helper = new SCPMakerHelper(_docs,_m);  // this is expensive
        }
        for (int j=0; j<clusters.size(); j++) {
          ...
          // System.err.println("column "+column_counter+" filled with "+clusterj.size()+" ones.");  // itc: HERE rm asap
          // Vector cjdocs = new Vector();
          cjdocs.clear();
          wgtsi.clear();
          for (int l = 0; l < clusterj.size(); l++) {
            ...
            if (weights!=null) wgtsi.add(new Double(weights[did.intValue()]));
          }
          // if the cost evaluator has been passed in, use it
          // else compute it
          Evaluator costseval = (Evaluator) getParam("evaluator");
          if (costseval!=null && costseval instanceof EnhancedEvaluator) {
            double costj = ((EnhancedEvaluator) costseval).evalCluster(clusterj);
            _c.setQuick(column_counter,costj);
          }
          else {
            double[] wsi = wgtsi.size()>0 ? new double[wgtsi.size()] : null;
            for (int ii=0; ii<wgtsi.size(); ii++) 
              wsi[ii] = ((Double)wgtsi.get(ii)).doubleValue();
            Vector centerjcosted = Document.getCenterCosted(cjdocs, _m, wsi);
            double cost = ( (Double) centerjcosted.elementAt(1)).doubleValue();
            // test if column must be penalized
            if (p != null) {
              int pval = p.intValue();
              boolean cont = true;
              for (int jj = 0; jj < clusterj.size() && cont; jj++) {
                Vector pnborsjj = helper.getKNN(jj, pval);
                for (int jj2 = 0; jj2 < pnborsjj.size(); jj2++) {
                  Integer jj2ind = (Integer) pnborsjj.elementAt(jj2);
                  if (clusterj.contains(jj2ind) == false) {
                    cost *= 100.0;
                    // if (cost < 1e-6) cost = 100;  // cost of singleton clusters
                    cont = false;
                    break;
                  }
                }
  ...
  public int[] convertSolution(int[] scpsol, double[] wgts) throws PartitioningException {
    ...
    try {
      ...
      Hashtable Xcwgts = new Hashtable();  // map<Integer Xindex, Double sumw>
      int index=0;
      for (int i=0; i<X.size(); i++) {
        if (X.getQuick(i)==1.0) {
          Xindices.put(new Integer(i), new Integer(index++));
          Vector Xdocs = new Vector();
          ArrayList wgtsi = wgts==null ? null : new ArrayList();
          for (int j=0; j<_A.rows(); j++) {
            if (_A.getQuick(j,i)>0) {
              Xdocs.addElement(_docs.elementAt(j));
              if (wgtsi!=null) wgtsi.add(new Double(wgts[j]));
            }
          }
          double[] wsi = wgtsi==null ? null : new double[wgtsi.size()];
          double sumw = 0.0;
          if (wsi!=null) {
            for (int j=0; j<wsi.length; j++) {
              wsi[j] = ((Double)wgtsi.get(j)).doubleValue();
              sumw += wsi[j];
            }
          }
          Xcwgts.put(new Integer(i), new Double(sumw));
          Document center = Document.getCenter(Xdocs, wsi);
      ...
      for (int row=0; row<_A.rows(); row++) {
        ...
        if (num_apps>1.5) {  // used to be 1.0
          // we have duplicates
            ...
            if (col_i.getQuick(j)==1) {
              ...
              if (dups==null) {
                ...
                if (clusterindices[j]>=0) 
                    throw new ClustererException("j="+j+"-doc is already assigned?");
                ...
                while (it.hasNext()) {
                  ...
                  if (col_id.intValue() == bestcol.intValue()) 
                    continue;  // don't remove dj from its assigned center
                  Document col_center = (Document) Xcenters.get(col_id);
                  Double col_sumw = (Double) Xcwgts.get(col_id);
                  if (col_sumw==null) {
                      throw new ClustererException("null col_sumw for col_id="+col_id);
                  }
                  double csw = col_sumw.doubleValue();
                  DoubleMatrix1D col = _A.viewColumn(col_id.intValue());
                  int col_size = (int) col.zSum();
                  if (col_size<2) continue;  // should not happen
                  double cs1 = col_sumw==null ? 
                                 (double) (col_size-1) / (double) col_size :
                                 (csw - wgts[j]) / csw;
                  col_center.div(cs1);
                  if (wgts==null) col_center.addMul(-1.0/((double)(col_size-1)), d);
                  else col_center.addMul(-wgts[j]/(csw-wgts[j]), d);
                  Xcenters.put(col_id, col_center);
                  // update Xcwgts too
                  double csw_upd = csw-wgts[j];
                  Xcwgts.put(col_id, new Double(csw_upd));
                }
                // done updating Xcenters+Xcwgts
      ...
      // DEBUG: evaluate solution AS-IS
      // sum up the costs from the chosen X's to see if they make sense
      double scp_val = 0;
      for (int i=0; i<_A.columns(); i++) {
          if (scpsol[i]==1) scp_val += _c.getQuick(i);
      }
      System.err.println("SCPMaker1.convertSolution(): GUROBI-based VALUE="+scp_val);
      // now compute SPP value
      KMeansSqrGuarantClusterer test_cl = new KMeansSqrGuarantClusterer();
      test_cl.setParams(_params);
      test_cl.addAllDocuments(_docs);
      // add centers
      /*
      Vector centers = new Vector();
      Iterator xcit = Xcenters.keySet().iterator();
      while (xcit.hasNext()) {
          centers.add(Xcenters.get(xcit.next()));
      }
      test_cl.setInitialClustering(centers);
      */
      Vector centers = Document.getCenters(_docs, clusterindices, k, wgts);
      test_cl.setInitialClustering(centers);
      test_cl.setClusteringIndices(clusterindices);
      KMeansSqrEvaluator test_eval = new KMeansSqrEvaluator();
      double spp_val = test_eval.eval(test_cl);
      System.err.println("SCPMaker1.convertSolution(): SPP SOLUTION VALUE="+spp_val);
  ...
  public void addOverExistingColumns(Vector clusters,
    ...
    try {
      ...
      // check if clusters that contain points for which their p-NN are
      // not also contained in the cluster should be heavily penalized
      Integer p2 = (Integer) _params.get("nn_in_cluster_threshhold");
      SCPMakerHelper helper = null;
      if (p2!=null) {
        helper = new SCPMakerHelper(_docs,_m);  // this is expensive
      }
      ...
      for (int i=0; i<clusters.size(); i++) {
        ...
        while (cur_col<scpsolution.length && scpsolution[cur_col]==0) {
          ...
          if (i==0 &&
            ...
            ArrayList dwgtsi = new ArrayList();
            ...
            if (docs2move.size() > 0) {
              ...
              if (cur_col2 < _A.columns() && cont == false) {
                ...
                // if the cost evaluator of each column has been passed in, use it
                // else compute it
                if (costseval!=null && costseval instanceof EnhancedEvaluator) {
                  double costj = ((EnhancedEvaluator) costseval).evalCluster(iidocs);
                  _c.setQuick(cur_col,costj);
                }
                else {
                  double[] wsi = wgtsi.size()>0 ? new double[wgtsi.size()] : null;
                  for (int jj=0; jj<wgtsi.size(); jj++)
                      wsi[jj] = ((Double)wgtsi.get(jj)).doubleValue();
                  Vector centerjcosted = Document.getCenterCosted(cidocs, _m, wsi);
                  double cost = ( (Double) centerjcosted.elementAt(1)).
                      doubleValue();
                  // test if column must be penalized
                  if (p2 != null) {
                    int pval = p2.intValue();
                    boolean cont2 = true;
                    for (int jj = 0; jj < iidocs.size() && cont2; jj++) {
                      Vector pnborsjj = helper.getKNN(jj, pval);
                      for (int jj2 = 0; jj2 < pnborsjj.size(); jj2++) {
                        Integer jj2ind = (Integer) pnborsjj.elementAt(jj2);
                        if (iidocs.contains(jj2ind) == false) {
                          cost *= 100.0;
                          // if (cost < 1e-6) cost = 100;  // cost of singleton clusters
                          cont2 = false;
                          break;
                ...
                Vector idocs2move2 = new Vector(idocs2move);  // auxiliary Vector
                for (int j = 0; j < _A.rows(); j++) {
                  if (_A.getQuick(j, cur_col3) > 0.0) {
                    Document dj = (Document) _docs.elementAt(j);
                    docs2move.addElement(dj);
                    dwgtsi.add(new Double(weights[j]));
                    idocs2move2.addElement(new Integer(j));
                  }
                }
                // if the cost evaluator of each column has been passed in, use it
                // else compute it
                if (costseval!=null && costseval instanceof EnhancedEvaluator) {
                  double costj = ((EnhancedEvaluator) costseval).evalCluster(idocs2move2);
                  _c.setQuick(cur_col3,costj);
                }
                else {
                  double[] wsi = dwgtsi.size()>0 ? new double[dwgtsi.size()] : null;
                  for (int jj=0; jj<dwgtsi.size(); jj++) {
                      wsi[jj] = ((Double)dwgtsi.get(jj)).doubleValue();
                  }
                  Vector centerjcosted = Document.getCenterCosted(docs2move, _m, wsi);
                  double cost = ( (Double) centerjcosted.elementAt(1)).
                      doubleValue();
                  // test if column must be penalized
                  if (p2 != null) {
                    int pval = p2.intValue();
                    boolean cont2 = true;
                    for (int jj = 0; jj < idocs2move.size() && cont2; jj++) {
                      Vector pnborsjj = helper.getKNN(jj, pval);
                      for (int jj2 = 0; jj2 < pnborsjj.size(); jj2++) {
                        Integer jj2ind = (Integer) pnborsjj.elementAt(jj2);
                        if (idocs2move.contains(jj2ind) == false) {
                          cost *= 100.0;
                          // if (cost < 1e-6) cost = 100;  // cost of singleton clusters
                          cont2 = false;
                          break;
        ...
        // else compute it
        if (costseval!=null && costseval instanceof EnhancedEvaluator) {
          double costj = ((EnhancedEvaluator) costseval).evalCluster(clusteri);
          _c.setQuick(cur_col,costj);
        }
        else {
          Vector cidocs = new Vector();
          ...
          if (p2 != null) {
            int pval = p2.intValue();
            boolean cont2 = true;
            for (int jj = 0; jj < clusteri.size() && cont2; jj++) {
              Vector pnborsjj = helper.getKNN(jj, pval);
              for (int jj2 = 0; jj2 < pnborsjj.size(); jj2++) {
                Integer jj2ind = (Integer) pnborsjj.elementAt(jj2);
                if (clusteri.contains(jj2ind) == false) {
                  cost *= 100.0;
                  // if (cost < 1e-6) cost = 100;  // cost of singleton clusters
                  cont2 = false;
                  break;
