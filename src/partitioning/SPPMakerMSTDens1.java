...
public class SPPMakerMSTDens1 implements SCPMakerIntf {
  ...
  public SPPMakerMSTDens1() {
    _params = new Hashtable();
  }


  /**
   * create a Set Partitioning Problem using the documents passed in
  ...
  public void createSCP(Vector docs, Vector clusters, int k,
    ...
    try {
      ...
      // MSTDensityEvaluator mstdeval = new MSTDensityEvaluator();
      EnhancedEvaluator mstdeval = (EnhancedEvaluator) getParam("ggadevaluator");
      mstdeval.setParams(_params);  // make sure params are set properly for the guy
      for (int i = 0; use_clusters_only==false && i < docsize && column_counter < _A.columns(); i++) {
        ...
        div.addElement(di);
        double cdi = mstdeval.evalCluster(div);
        ...
        for (j = 1; j <= clusters_per_doc && column_counter < _A.columns(); j++) {
          ...
          // compute cost
          cost = mstdeval.evalCluster(nnbors);
      ...
      if (clusters!=null) {
        for (int j=0; j<clusters.size(); j++) {
          ...
          double cost;
          // compute cost
          // cost = (new MSTDensityEvaluator()).evalCluster(cjdocs);
          EnhancedEvaluator eval = (EnhancedEvaluator) getParam("ggadevaluator");
          eval.setParams(_params);
          cost = eval.evalCluster(cjdocs);
          _c.setQuick(column_counter, cost);
          column_counter++;
        }
      }
   ...
   * @return int[]
   */
  public int[] convertSolution(int[] scpsol, double[] weights) throws PartitioningException {
    ...
    try {
      ...
      System.out.println("SPP problem solution statistics: min_col_sz = "+min_col_sz + " max_col_sz = "+max_col_sz);  // itc: HERE
          ...
          if (i==0 &&
            ...
            if (docs2move.size() > 0) {
              ...
              if (cur_col2 < _A.columns() && cont == false) {
                ...
                // double cost = (new MSTDensityEvaluator()).evalCluster(cidocs);
                EnhancedEvaluator eval = (EnhancedEvaluator) getParam("ggadevaluator");
                eval.setParams(_params);
                double cost = eval.evalCluster(cidocs);
                ...
                // cost = (new MSTDensityEvaluator()).evalCluster(docs2move);
                cost = eval.evalCluster(docs2move);
  ...
  public Vector expand(Vector centers, int[] asgns, int factor) throws PartitioningException {
    System.err.println("sppmakermstdens1.expand(...,"+factor+") started.");
    ...
    System.err.println("sppmakermstdens1.expand(...): added "+results.size()+" columns.");
    return results;
  }
