...
public class KMeansClusterer implements Clusterer {
  ...
  public KMeansClusterer() {
    _evaluator = new KMeansEvaluator();
  ...
  public Vector clusterDocs() throws Exception {
    ...
    while (!ct.isDone()) {
      num_cur_iters++;
      // System.err.println("KMeansClusterer.clusterDocs(): start new loop...");
      ...
      for (int i=0; i<k; i++) {
        ...
        if (v.size()>0 || project_on_empty)
          centers_new.set(i, Document.getCenter(v, null));
        else centers_new.set(i, _centers.elementAt(i));
        // make sure cluster center remains, even with no data points associated
      }
...
/*
      // itc: HERE print centers
      System.err.println("KMeansClusterer.clusterDocs(): printing _centers: ");
      for (int i=0; i<_centers.size(); i++) {
        Document ci = (Document) _centers.elementAt(i);
        System.err.print("_centers["+i+"]="+ci+" ");
      }
      System.err.println("");
      System.err.print("KMC.clusterDocs(): printing _clusterIndices: ");
      for (int i=0; i<ind.length; i++) {
        System.err.print("i["+i+"]="+ind[i]+" ");
      }
      System.err.println("");
      // itc: HERE up to here
*/
    }
    // System.err.println("KMeansClusterer.clusterDocs(): ended loop...");
