...
public class SDKMeansSqrClusterer implements Clusterer {
  ...
  public Vector clusterDocs() throws Exception {
    ...
    while (!ct.isDone()) {
      ...
      System.err.println("SDKMeansSqrClusterer: start new iteration");
      ...
      for (int i=0; i<n; i++) {
        ...
        for (int l=0; l<k; l++) {
          ...
          double v_li = (numi[l]/((double) numi[l]+1))*dists[i][l] -
                        (numi[p]/((double) numi[p]-1))*dists[i][p];
          if (v_li<best_gain) {
            best_gain = v_li;
            best_ind = i;
            target_ind = l;
