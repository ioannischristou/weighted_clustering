 ...
 * The objective value of each set (column) is basically the cluster validity as
 * implemented in GBMiniMaxEvaluator and described in "Pattern Recognition" by
 * Theodoridis and Koutroubas, 3rd ed.
 * It is used to populate a vector c -a DoubleMatrix1D object.
 *
 ...
 * @version 1.0
 */
public class SPPGBMiniMaxClusterer implements Clusterer {
  ...
  public Vector clusterDocs() throws ClustererException {
    ...
    try {
      ...
      EnhancedEvaluator eval = new GBMiniMaxEvaluator();
      eval.setMasterDocSet(_docs);
      ...
      if (init_imp_val<Double.MAX_VALUE) {
        ...
        // itc: HERE rm asap
        int cards[] = improver.getClusterCards();
        for (int i=0; i<cards.length; i++) {
          System.out.print("c["+i+"]="+cards[i]+" ");
        }
        System.out.println("");
      ...
      {
        System.err.println("SPPGBMiniMaxClusterer.clusterDocs(): new iteration");
        ...
        System.err.println("SPPGBMiniMaxClusterer.clusterDocs(): done creating spp.mps file.");  // itc: HERE rm asap
        // call SCIP solver
        ...
        if (retcode!=0)
          throw new ClustererException("SPPGBMiniMaxClusterer.clusterDocs(): exec() SCIP failed...");
      ...
      e.printStackTrace();
      throw new ClustererException("SPPGBMiniMaxClusterer: clustering failed...");
    }
    if (_clusterIndices==null) {
      throw new ClustererException("null _clusterIndices after running clusterDocs()");
    }
    System.err.println("SPPGBMiniMaxClusterer.clusterDocs(): total writing time="+total_writing_time+" msecs.");
    return _centers;
  }
