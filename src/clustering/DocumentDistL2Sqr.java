...
public class DocumentDistL2Sqr extends DocumentDist {
  public DocumentDistL2Sqr() {
  }
  ...
  public double dist(Document x, Document y) throws ClustererException {
    if (x.getDim()!=y.getDim())
      throw new ClustererException("DocumentDistL2Sqr: docs of dif. size");
    Iterator x_iter = x.entrySetIterator();  // returns Entry.Map objects
    Iterator y_iter = y.entrySetIterator();
    double tot=0.0;
    if (x_iter.hasNext()==false) return l2dist2(y_iter);
    if (y_iter.hasNext()==false) return l2dist2(x_iter);
    ...
    while (true) {
      if (x_pos == y_pos) {
        ...
        if (x_iter.hasNext() == false) {
          return (tot + l2dist2(y_iter)); // done
        }
        if (y_iter.hasNext() == false) {
          return (tot + l2dist2(x_iter)); // done
        }
  ...
  private double l2dist2(Iterator vals) {
    ...
    while (vals.hasNext()) {
      ...
      double v = ((Double) me.getValue()).doubleValue();
      res += (v*v);
    }
    return res;
  }
  ...
  /**
   * Deprecated method of dist(x,y) as it's not thread-safe for some reason...
   * @param x Document
   * @param y Document
   * @throws ClustererException
   * @return double
  ...
  public double distOld(Document x, Document y) throws ClustererException {
    ...
    double tot=0.0;
    if (x_posit.hasNext()==false) return l2dist2Old(y_valsit);
    if (y_posit.hasNext()==false) return l2dist2Old(x_valsit);
    ...
    while (true) {
      if (x_pos==y_pos) {
        ...
        if (x_posit.hasNext()==false)
          return tot+l2dist2Old(y_valsit);  // done
        if (y_posit.hasNext()==false)
          return tot+l2dist2Old(x_valsit);  // done
        ...
        // advance x
        if (x_posit.hasNext()==false) return tot+l2dist2Old(y_valsit);  // done
        x_pos = ((Integer) x_posit.next()).intValue();
        x_val = ((Double) x_valsit.next()).doubleValue();
      }
      else {  // x_pos>y_pos
        // advance y
        if (y_posit.hasNext()==false) return tot+l2dist2Old(x_valsit);  // done
        y_pos = ((Integer) y_posit.next()).intValue();
        y_val = ((Double) y_valsit.next()).doubleValue();
      }
    }
