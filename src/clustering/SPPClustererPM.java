 ...
 * @version 1.0
 */
public class SPPClustererPM implements Clusterer {
  ...
  public Vector clusterDocs() throws ClustererException {
    ...
    try {
      ...
      if (improver==null) improver = new KMeansGuarantClusterer();
      Evaluator eval = (Evaluator) _params.get("evaluator");
      if (eval==null) eval = new KMeansEvaluator();
      ...
      while (cont) {
        ...
        if (improve_via_kmeans) {
          ...
          if (improver==null)
            improver = new KMeansGuarantClusterer();
          improver.reset();
          improver.addAllDocuments(_docs);
          improver.setParams(_params);
          ...
          double val = eval.eval(improver);
          System.out.println(
              "SPPClustererPM.clusterDocs(): after improver2 improvement value = " +
              val);
          _centers = improver.getCurrentCenters();
          _clusterIndices = improver.getClusteringIndices();  // itc: HERE ensure improvements go to the solution
          // improve via SD-Means
          ...
          if (use_SD) {
            ...
            // that the improvements make it to the SPPClustererPM results
            // System.err.println("improving soln via SD");
            val = eval.eval(improver2);
            System.out.println(
                "SPPClustererPM.clusterDocs(): after SD improved value=" + val);
          }
          if (old_val > val) {
            new_clusters.clear();
            new_clusters.addAll(improver.getIntermediateClusters());
