...
public class KMeansSqrGuarantClusterer implements Clusterer {
  ...
  public KMeansSqrGuarantClusterer() {
    ...
    while (!ct.isDone()) {
      ...
      for (int i=0; i<k; i++) {
        ...
        if (v.size()>0 || project_on_empty) {
          double[] wsi = wgts == null ? null : new double[wgtsi.size()];
          if (wsi!=null) {
      ...
      */
      ...
      else if (new_val > best_val) {  // sanity check
          throw new ClustererException("best_val up to now="+best_val+
                                       " but latest iteration has new_val="+
                                       new_val);
      }
    }
    if (_clusterIndices==null) {
